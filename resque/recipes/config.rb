node[:deploy].each do |app_name, deploy|
  next unless node[app_name].present? && node[app_name][:resque].present?
  if platform?('ubuntu')
    user = 'www-data'
  ...
  resque_env['VVERBOSE'] = '1' if node[:resque][:verbose]
  ...
  resque_scheduler_env['VVERBOSE'] = '1' if node[:resque][:verbose]

  next unless node[app_name][:resque][:scheduler]
